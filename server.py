...
def load_corpus(docs: list[str]):
    global documents, tokens, vocabulary, tfidf_matrix

    documents = [d.strip().lower() for d in docs if d.strip()]
    tokens = [doc.split() for doc in documents]
    vocabulary = sorted(set(w for doc in tokens for w in doc))
...
def bag_of_words(text: str):
    ...
    vector = [1 if w in words else 0 for w in vocabulary]
    return vector
